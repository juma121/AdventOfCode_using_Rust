fn main() {
    ...
    let mut detail_vec: Vec<Passworddetails> = Vec::new();
    ...
    for element in v {
        ...
        let mut previous_chars = String::new();

        let element_iter = element.chars();

        for c in element_iter {
            if c.eq(&"-".chars().next().unwrap()) {
                low.push_str(previous_chars.as_str());
                previous_chars.clear();
            } else if c.eq(&":".chars().next().unwrap()) {
                if previous_chars.len() == 2 {
                    letter.push(previous_chars.chars().last().expect("could not acces last"));
                    high.push(
                        previous_chars
                            .chars()
                            .next()
                            .expect("could not access last"),
                    );
                    previous_chars.clear();
                } else if previous_chars.len() == 3 {
                    letter.push(previous_chars.chars().last().unwrap());
                    high.push(previous_chars.chars().next().unwrap());
                    high.push(previous_chars.chars().skip(1).next().unwrap());
                    previous_chars.clear();
                }
            } else {
        ...
        password.push_str(previous_chars.as_str());

        let details = Passworddetails {
            low: low.parse::<i8>().unwrap(),
            high: high.parse::<i8>().unwrap(),
            letter: letter.chars().next().unwrap(),
            password: password,
        };

        detail_vec.push(details);
    }

    let mut valid_counter = 0;
    for detail in detail_vec {
        if validate_new_password(detail.low, detail.high, detail.letter, detail.password) {
            valid_counter += 1;
        }
    }
...
fn validate_password(l: i8, h: i8, letter: char, password: String) -> bool {
    let mut counter = 0;
...
fn validate_new_password(l: i8, h: i8, letter: char, password: String) -> bool {
    let mut index = 1;
    let mut matches = 0;
    for c in password.chars() {
        if c.eq(&letter) && (l == index || h == index) {
            matches += 1;
            index += 1;
        } else {
            index += 1;
