fn main() {
    ...
    let edit_str = original_str.replace(" ", "").replace("\n\n", ",");

    let mut str_vec: Vec<String> = fill_vec_with_data_str(&edit_str);
    let counter = validate_str(&str_vec);
    println!("Valid: {}", counter);
}

fn fill_vec_with_data_str(data_str: &String) -> Vec<String> {
    let mut str_buffer = String::new();
    let mut str_vec: Vec<String> = Vec::new();

    for c in data_str.chars() {
        if c.eq(&',') {
            str_vec.push(str_buffer.clone());
            str_buffer.clear();
        } else {
            str_buffer.push(c);
        }
    }
    str_vec.push(str_buffer);

    //let vec_ordered_data: Vec<Passportdata> = get_ordered_data(str_vec);
    get_ordered_data(&str_vec);
    str_vec
}

fn get_ordered_data(str_vec: &Vec<String>) {
    let mut buffer_struct = Passportdata {
        byr: 0,
        iyr: 0,
        eyr: 0,
        hgt: None,
    ...
    for element in str_vec {
        buffer_struct.byr = get_byr(&element);
        buffer_struct.eyr = get_eyr(&element);
        buffer_struct.iyr = get_iyr(&element);
        buffer_struct.hgt = get_hgt(&element);
        buffer_struct.hcl = get_hcl(&element);
    }
}

fn get_hcl(s: &String) -> Option<String> {
    let mut buffer_str = String::new();
    let mut sub_str = String::new();
    let y = s.matches(pat)

    let hcl = s.strip_prefix("hcl:")?;
    
    let mut x = 0;
    for c in hcl.to_string().chars() {
        if c.eq(&'#') {
            buffer_str.push(c);
            x += 1
        } else if x < 7  && c.is_numeric() ||  x < 7 && c.is_alphabetic() && (c >= 'a' || c <= 'f') {
            buffer_str.push(c);
            x += 1
        } else {
            break;
...
fn get_hgt(s: &String) -> Option<String> {
    let byr = s.find("hgt:");
    let mut sub_str = String::new();
    if let Some(byr) = s.find("hgt:") {
        sub_str = s.get(byr..(byr + 9))?.to_string();
        let hgt = sub_str.trim_start_matches("hgt:");
        if validate_hgt(&hgt) {
            return Some(hgt.to_string());
        }
    }
    Some("".to_string())
}
...
fn validate_hgt(s: &str) -> bool {
    let mut flag = false;
    for c in s.to_string()[0..3].to_string().chars() {
        if c.is_numeric() {
            flag = true;
        } else {
            flag = false;
    ...
    if s.to_string()[3..].contains("in") || s.to_string()[3..].contains("cm") {
        flag = true;
    } else {
        flag = false;
    }
...
fn get_iyr(s: &String) -> i32 {
    let byr = s.find("iyr:");
    let mut sub_str = String::new();
    if let Some(byr) = s.find("iyr:") {
        sub_str = s[byr..(byr + 8)].to_string();
        let year = sub_str.trim_start_matches("iyr:");
        let y_int = year.parse::<i32>().expect("could not parse the year");
        return y_int;
    }
    0
}

fn get_eyr(s: &String) -> i32 {
    let byr = s.find("eyr:");
    let mut sub_str = String::new();
    if let Some(byr) = s.find("eyr:") {
        sub_str = s[byr..(byr + 8)].to_string();
        let year = sub_str.trim_start_matches("eyr:");
        let y_int = year.parse::<i32>().expect("could not parse the year");
        return y_int;
    }
    0
...
fn get_byr(s: &String) -> i32 {
    let byr = s.find("byr:");
    let mut sub_str = String::new();
    if let Some(byr) = s.find("byr:") {
        sub_str = s[byr..(byr + 8)].to_string();
        let year = sub_str.trim_start_matches("byr:");
        let y_int = year.parse::<i32>().expect("could not parse the year");
        return y_int;
    }
    0
...
fn validate_str(str_vec: &Vec<String>) -> i32 {
    let mut count_valid = 0;
    for element in str_vec {
        if element.contains("byr")
            && element.contains("iyr")
            && element.contains("eyr")
            && element.contains("hgt")
            && element.contains("hcl")
            && element.contains("ecl")
            && element.contains("pid")
        {
            count_valid += 1;
        }
    }
